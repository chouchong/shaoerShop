...
$(function() {
	...
	var tempCids = $("#categoryHistoryList").find("option:selected")
			.attr('cid');
		...
		success : function(res) {
			...
			if (eval(res) != -1) {
				...
				for (var i = 0; i < goods_category_quick.length; i++) {
					...
					str += "<li onclick='quick(&#39;" + quick_id + "&#39;,&#39"
							+ quick_name + "&#39)'>";
					str += "<span>" + quick_name.replace(/:/g, "&nbsp;&gt;")
							+ "</span>";
					// str += "<a href='javascript:;'
					// title='删除'>X</a>";//由于用的是Cookie，会定时删除
					str += "</li>";
				}
			} else {
...
function quick(quick_id, quick_name) {
	...
	$('#commListArea').hide();
	$("#commListArea").prev().html(quick_name);
	...
	$.ajax({
		...
		success : function(res) {
			if (res != null && res.length != 0) {
				var $categoryDiv = CreateSelectCatgoryDiv(230, 300,
						(parseInt(index) + 1));
...
function nextPage() {
	var dis = $("#nextPage").attr("disabled");
	if (dis == "disabled") {
		return;
	}
...
function GetProductCategoryData(categoryData) {
	...
	for (var i = 0; i < categoryData.length; i++) {
		...
		if (leaf == 0) {
			$rootCategory = "<div id='"
					+ cateId
					+ "' class='categoryItem' onclick='ClickHasSubCategory(this)'><span style='width: 170px;'>"
...
function ClickHasSubCategory(event) {
	...
	$.ajax({
		...
		success : function(res) {
			if (res != null && res.length != 0) {
				var $categoryDiv = CreateSelectCatgoryDiv(230, 300,
						(parseInt($currentSelectCategoryDivIdNum) + 1));
				$("#categoryDivContainer").append($categoryDiv);
				var $data = GetProductCategoryData(res);
				$categoryDiv.append($data);
	...
	if ($allSelectedCategory.length > 0) {
		for (var i = 0; i < $allSelectedCategory.length; i++) {
			...
			if ($allSelectedCategory[i].id >= $currentSelectCategoryDivIdNum) {
				$(
						".hasSelectedCategoryDiv span[id="
								+ $allSelectedCategory[i].id + "]").remove();
			}
		}
	}
...
function CreateSelectCatgoryDiv(w, h, num) {
	var $selectCategoryDiv = $("<div id='selectCategoryDiv" + num
			+ "' class='selectCategoryDiv'></div>");
	$selectCategoryDiv.css("width", w + "px");
	$selectCategoryDiv.css("height", h + "px");
	$selectCategoryDiv
			.append('<div class="category-search"><i class="icon-search-tabao" style="background: url('+ADMINIMG+'/SelectCategory_Search.png) no-repeat -4px -2px;"></i><input type="text" name="search_category" placeholder="输入名称"></div>')
