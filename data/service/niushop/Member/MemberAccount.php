...
use data\model\system\ConfigModel;
...
//                         $retval = $shop_account->addShopCoinAccountRecords(0, 2, $number*(-1), $data_id, $text);
//                         if(!($retval > 0))
//                         {
//                             $member_account->rollback();
//                             return LOW_COIN;
...
//         var_dump($convert_rate);
    ...
    public function getCoinConvertRate(){
        ...
        $config_rate = $config->getInfo(['oauth_key' => 'COIN_CONFIG'], '*');
        if(empty($config_rate))
        {
            return 1;
        }else{
            $rate = json_decode($config_rate['oauth_value'], true);
