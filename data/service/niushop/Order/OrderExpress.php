...
class OrderExpress extends BaseService{
     ...
     * @param unknown $order_goods_id_array   订单项数组
     * @param unknown $express_name           包裹名称
     * @param unknown $shipping_type          发货方式1 需要物流 0无需物流
     * @param unknown $express_company_id     物流公司ID
     * @param unknown $express_no             物流单号
     */
    public function delivey($order_id, $order_goods_id_array, $express_name, $shipping_type, $express_company_id, $express_no)
    {
        $user = new UserModel();
        $user_name = $user->getInfo(['uid' => $this->uid], 'user_name');
