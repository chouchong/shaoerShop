...
use data\model\niushop\NsOrderModel;
use data\model\niushop\NsOrderActionModel as NsOrderActionModel;
...
use data\model\niushop\NsOrderGoodsExpressModel;
...
use data\model\niushop\NsOrderPromotionDetailsModel;
 ...
 // 订单主表
    ...
    public function orderCreate($order_type, $out_trade_no, $pay_type, $shipping_type, $order_from, $buyer_ip, $buyer_message, $buyer_invoice, $shipping_time, $receiver_mobile, $receiver_province, $receiver_city, $receiver_district, $receiver_address, $receiver_zip, $receiver_name, $point, $coupon_id, $user_money, $goods_sku_list, $platform_money, $coin)
    {
        ...
        try {
            ...
            $deliver_price = $order_goods_express->getSkuListExpressFee($goods_sku_list, $receiver_province, $receiver_city);
            // 订单商品费用
...
;
            ...
            $pay = new UnifyPay();
            $pay->createPayment($shop_id, $out_trade_no, "niushop订单", "niushop订单", $pay_money, 1, $order_id);
            //满额包邮活动
...
;
            ...
            foreach ($list as $k => $v) {
                ...
                if (! empty($mansong_data)) {
                    ...
                    if ($mansong_data['give_coupon'] != 0) {
                        ...
                        $member_coupon->UserAchieveCoupon($order_info['buyer_id'], $mansong_data['give_coupon']);
                    }
                }
            }
        }
    ...
    public function orderClose($orderid)
    {
        ...
        try {
            ...
            $order_info = $this->order->getInfo([
                'order_id' => $orderid
            ], 'point, coupon_id, user_money, buyer_id,shop_id,user_platform_money, coin_money');
            // 积分返还
            $account_flow = new MemberAccount();
            ...
            if ($order_info['coin_money'] > 0) {
                ...
                $account_flow->addMemberAccountData($order_info['shop_id'], 1, $order_info['buyer_id'], 1, $order_info['coin_money'] / $coin_convert_rate, 2, $orderid, '订单关闭返还积分');
            }
            // 会员余额返还
            ...
            if ($order_info['user_platform_money'] > 0) {
                $account_flow->addMemberAccountData(0, 1, $order_info['buyer_id'], 1, $order_info['user_platform_money'], 2, $orderid, '商城订单关闭返还平台余额');
            }
            // 优惠券返还
            $coupon = new MemberCoupon();
    ...
    public function orderGoodsRefundFinish($order_id)
    {
        ...
        try {
            ...
            if ($refunding_count > 0) {
                ...
                $orderInfo->order_status = OrderStatus::getOrderCommonStatus()[7]['status_id']; // 已退款
    ...
    public function getDetail($order_id)
    {
        ...
        if ($order_detail['payment_type'] == 1) {
            $order_detail['payment_type_name'] = '微信支付';
        } else 
            if ($order_detail['payment_type'] == 2) {
                $order_detail['payment_type_name'] = '支付宝支付';
            } else 
                if ($order_detail['payment_type'] == 10) {
                    $order_detail['payment_type_name'] = '线下支付';
                } else {
                    $order_detail['payment_type_name'] = '';
                }
        if ($order_detail['shipping_type'] == 1) {
            $order_detail['shipping_type_name'] = '需要物流';
        } else 
            if ($order_detail['shipping_type'] == 0) {
                $order_detail['shipping_type_name'] = '无需物流';
            } else {
        ...
        // 查询订单操作日志
        $order_action = new NsOrderActionModel();
        $order_action_log = $order_action->all([
            'order_id' => $order_id
        ]);
