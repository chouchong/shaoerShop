...
class Send extends \think\Model
{
    ...
    function __construct(){
        ...
        static ::$sms_config= [
    		'appkey'		=> $config['oauth_value']['appKey'],//阿里大于APPKEY
    		'secretKey' 	=> $config['oauth_value']['secretKey'],//阿里大于secretKey
    		'FreeSignName' 	=> $config['oauth_value']['freeSignName'],//短信签名
	     ];
    }
	...
	public function email($toemail, $title, $content){
	    ...
	    $mail = new Email();
	    $mail->setServer($info['oauth_value']['email_host'], $info['oauth_value']['email_id'], $info['oauth_value']['email_pass']);
	    $mail->setFrom($info['oauth_value']['email_addr']);
