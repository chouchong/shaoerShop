	...
	if (j > 0) {
		$.ajax({
			...
			success : function(res){
				location.href= shop_main + "/member/buyflowsecondgetGoods";
			}
		});
//		formobj.action = shop_main + "/member/buyflowsecondgetgoods";// 下一步，待付款订单
//		$("#shop_id").val(shop_id[0]);// 单店铺id
//		$("#cart_id").val(cart_id_arr);
//		formobj.submit();
	}
}
...
function updateGoodsNumber(cart_id, num) {
	if (null != cart_id && null != num) {
		$.ajax({
			url : shop_main + "/Member/updateCartGoodsNumber",
			type : "post",
			async : true,
			data : {
				"cart_id" : cart_id,
...
function getCopeSum() {
	...
	for (j = 0; j < sel_goods.length; j++) {
		sum += parseInt($("#subtotal_" + sel_goods[j]).attr("data-total"));
...
function minus_num(cart_id, max_buy, stock) {
	...
	if (number > 1) {
		number--;
		obj.val(number);
		$("#subtotal_" + cart_id).text("￥" + number * price);
		$("#subtotal_" + cart_id).attr("data-total", number * price);
		getCopeSum();// 刷新应付总额
