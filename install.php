	...
	if($ispost) {
		...
		if(empty($error)) {
 		...
 		if(empty($error)) {
			...
			$insert_error = mysql_query("INSERT INTO sys_user (user_name, user_password, real_password, is_system, is_member, reg_time) 
			VALUES('{$user['username']}', '{$password}','{$user['password']}', '1', '1','" . $datetime . "')");			
			if($insert_error){
			    $insert_id = mysql_insert_id();
			    //添加管理员用户组
			    $group_list = array();
			    $group_string = "";
			    $result= mysql_query("SELECT * FROM sys_module where is_control_auth = 1");
			    ...
			    $group_error = mysql_query("INSERT INTO sys_user_group (group_name,instance_id, is_system, module_id_array, create_time)
			        VALUES('管理员组','1', '1','{$group_string}','" . $datetime . "')");
			        if($group_error){
	...
	<body style="background-color:#28b0e4;">
		<div class="container">
			...
			<div class="footer" style="margin:15px auto;">
				...
				<div class="text-center">
					Powered by <a   target = "_blank" href="http://www.niushop.com.cn"><b>Niushop开源商城</b></a> v1.0 B2C演示版 &copy; 2015-2025 <a target = "_blank" href="http://www.niushop.com.cn">www.niushop.com.cn</a>
				</div>
			</div>
		</div>
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<input class="form-control" type="text" name="db[name]" value="niushop_b2c">
					</div>
				</div>
			</div>
		</div>
...
// 	$themes = get_store_theme();
	...
	<div class="alert alert-success">
		恭喜您!已成功安装“Niushop开源商城商城”系统，您现在可以: <a target="_blank" class="btn btn-success" href="./index.php/shop">访问网站首页</a>
	</div>
	<div class="form-group">
		<h5><strong>Niushop开源商城商城</strong></h5>		
	</div>
