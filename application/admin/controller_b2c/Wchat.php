...
use data\service\system\WeixinMessage;
...
class Wchat extends BaseController
{
    ...
    public function config()
    {
        ...
        $call_back_url = $domain_name . \think\Request::instance()->root() . '/wap/wchat/relateWeixin';
        $this->assign("url", $url);
        $this->assign("call_back_url", $call_back_url);
        $this->assign('wchat_config', $wchat_config["oauth_value"]);
    ...
    public function updateMenuToWeixin()
    {
        ...
        if (NS_VERSION == NS_VER_B2C || NS_VERSION == NS_VER_B2C_FX) {
            ...
            $auth_info = $weixin->get($this->instance_id);
        }
        ...
        if (! empty($auth_info)) {
            if (NS_VERSION == NS_VER_B2C || NS_VERSION == NS_VER_B2C_FX) {
                ...
                $wchat_auth = new WchatOauth($auth_info['authorizer_appid']);
            }
    ...
    public function qrcode()
    {
        ...
        if (request()->isAjax()) {
            ...
            $template_url = 'upload/qrcode/qrcode_template/qrcode_template_' . $id . '_' . $this->instance_id . '.png';
            if ($id == 0) {
                ...
                showUserQecode('', 'upload/qrcode/thumb_template' . 'qrcode_' . $res . '_' . $this->instance_id . '.png', '', $shop_logo, '', $_POST, 'upload/qrcode/qrcode_template/qrcode_template_' . $res . '_' . $this->instance_id . '.png');
                $res = $weixin->updateWeixinQrcodeTemplate($res, $this->instance_id, $background, $nick_font_color, $nick_font_size, $is_logo_show, $header_left, $header_top, $name_left, $name_top, $logo_left, $logo_top, $code_left, $code_top, 'upload/qrcode/qrcode_template/qrcode_template_' . $res . '_' . $this->instance_id . '.png');
            } else {
                ...
                showUserQecode('', 'upload/qrcode/thumb_template' . 'qrcode_' . $id . '_' . $this->instance_id . '.png', '', $shop_logo, '', $_POST, 'upload/qrcode/qrcode_template/qrcode_template_' . $id . '_' . $this->instance_id . '.png');
            }
            return AjaxReturn($res);
        } else {
            ...
            $shop_info = $shop->getShopInfo($this->instance_id, 'shop_logo');
            $this->assign('shop_logo', $shop_info['shop_logo']);
    ...
    public function materialMessage()
    {
        ...
        if (request()->isAjax()) {
            ...
            $list = $weixin->getWeixinMediaList($pageIndex, PAGESIZE, $condition, 'create_time desc');
            return $list;
        }
        $this->assign('type', $type);
        $this->assign('child_menu_list', $child_menu_list);
