...
class Distribution extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $shop_config = $nfx_shop_config->getShopConfigDetail($this->shop_id);
    ...
    public function distributionCenter()
    {
        ...
        $this->assign('nick_name', $nick_name['nick_name']);
    ...
    public function homeRegionAgent()
    {
        ...
        if ($region_agent_info['agent_type'] > 2) {
            $address_info .= $address->getDictrictName($region_agent_info['agent_districtid']);
            $agent_name = '区代';
        }
    ...
    public function toWithdraw()
    {
        ...
        if (request()->isAjax()) {
            ...
            // 选择的账户
            $account_list = $nfx_user->getUserBankAccount();
            $this->assign('account_list', $account_list);
            // 佣金统计情况
            $user_account = $nfx_user->getNfxUserAccount($this->uid, $this->shop_id);
            $this->assign('user_account', $user_account);
            $shop = new ShopService();
            $withdraw_info = $shop->getWithdrawInfo($this->instance_id);
